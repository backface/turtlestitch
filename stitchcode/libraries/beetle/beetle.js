...
Beetle.prototype.addPositionToTrack = function () {
	this.track.push([
...
Beetle.prototype.getTrack = function () {
	return new List(this.track.map(pos => new List(pos)));
};
...
Beetle.prototype.changeRotations = function (x, y, z) {
    const newRotation = BABYLON.Quaternion.FromEulerAngles(radians(Number(x)) * -1, radians(Number(y)) * -1, radians(Number(z)) * -1);
	if (this.body.rotationQuaternion) {
		this.body.rotationQuaternion = newRotation.multiply(this.body.rotationQuaternion);
	} else {
		this.body.rotationQuaternion = newRotation;
	}
    this.controller.changed();
};

Beetle.prototype.rotateInTurtlePlane = function(degrees) {
    const localYAxis = new BABYLON.Vector3(0, 1, 0);
	const worldUpDirection = BABYLON.Vector3.TransformNormal(localYAxis, this.body.getWorldMatrix());
	const dRad = radians(degrees);
	const rotationAroundSelf = BABYLON.Quaternion.RotationAxis(worldUpDirection, dRad);
	if (this.body.rotationQuaternion) {
	    this.body.rotationQuaternion = rotationAroundSelf.multiply(this.body.rotationQuaternion);
	} else {
		this.body.rotationQuaternion = rotationAroundSelf;
	};
	this.controller.changed();
};

Beetle.prototype.turnAroundWorldAxis = function(axis, angle) {
    const angleRad = radians(angle * (-1));
    ...
    switch (axis.toLowerCase()) {
        case 'x':
            const radiusX = Math.sqrt(currentPos.y * currentPos.y + currentPos.z * currentPos.z);
            const angleX = Math.atan2(currentPos.z, currentPos.y) + angleRad;
            newPosition = new BABYLON.Vector3(
                currentPos.x,
                radiusX * Math.cos(angleX),
                radiusX * Math.sin(angleX)
            );
			rotationAxis = BABYLON.Axis.X;
			rotationQuat = BABYLON.Quaternion.RotationAxis(rotationAxis, angleRad);
            break;
            
        case 'y':
            const radiusY = Math.sqrt(currentPos.x * currentPos.x + currentPos.z * currentPos.z);
            const angleY = Math.atan2(currentPos.z, currentPos.x) + angleRad;
            newPosition = new BABYLON.Vector3(
                radiusY * Math.cos(angleY),
                currentPos.y,
                radiusY * Math.sin(angleY)
            );
			rotationAxis = BABYLON.Axis.Y;
			rotationQuat = BABYLON.Quaternion.RotationAxis(rotationAxis, angleRad*(-1));
            break;
            
        case 'z':
            const radiusZ = Math.sqrt(currentPos.x * currentPos.x + currentPos.y * currentPos.y);
            const angleZ = Math.atan2(currentPos.y, currentPos.x) + angleRad;
            newPosition = new BABYLON.Vector3(
                radiusZ * Math.cos(angleZ),
                radiusZ * Math.sin(angleZ),
                currentPos.z
            );
        ...
        default:
            throw new Error('should be "x", "y",or "z"');
    }
    ...
    if (this.body.rotationQuaternion) {
        this.body.rotationQuaternion = rotationQuat.multiply(this.body.rotationQuaternion);
    } else {
        this.body.rotationQuaternion = rotationQuat;
    }

    this.controller.changed();
};

Beetle.prototype._transformNormalByQuaternion = function(vector, quaternion) {
    const p = new BABYLON.Quaternion(0, vector.x, vector.y, vector.z);
    const qConjugate = quaternion.conjugate();
    const pRotated = quaternion.multiply(p).multiply(qConjugate);
    return new BABYLON.Vector3(pRotated.x, pRotated.y, pRotated.z);
};

Beetle.prototype.setGoWithTurtle = function (withTurtle) {
	this.goWithTurtle = withTurtle;
};

Beetle.prototype.ifGoWithTurtle = function () {
	return this.goWithTurtle;
};

Beetle.prototype.convertWorldToScreen = function(position, centerOrigin = true) {
    ...
    try {
        ...
        const worldPoint = new BABYLON.Vector3(position[0], position[1], -position[2]);
        
        const symmetricCameraPosition = new BABYLON.Vector3(
            -camera.position.x,
            camera.position.y, 
            -camera.position.z
        );
        
        const symmetricViewMatrix = BABYLON.Matrix.LookAtLH(
            symmetricCameraPosition,
            camera.target,
            new BABYLON.Vector3(0, 1, 0)
        );
        
        const transformMatrix = symmetricViewMatrix.multiply(camera.getProjectionMatrix());
        ...
        if (centerOrigin) {
            ...
            screenY = (canvas.height / 2 - projected.y);
        } else {
            screenX = projected.x;
            screenY = canvas.height - projected.y;
        }
        
        return new List([screenX, screenY]);
        
    } catch (error) {
        console.error("coordinates transfer error:", error);
        return null;
    }
};

Beetle.prototype.get2DTrackInScene = function () {
	return new List(this.track.map(pos => this.convertWorldToScreen(pos)));
};

BABYLON.ArcRotateCamera.prototype.rotateByXY = function (deltaX,deltaY) {
    if (!this.fpvEnabled) {
        this.inertialAlphaOffset = deltaX * -0.0005;
        this.inertialBetaOffset = deltaY * -0.001;
        this.framing = false;
...
SnapExtensions.primitives.set('bb_turnAroundWorldAxis(axis,degrees)', function (axis,degrees) {
    var stage = this.parentThatIsA(StageMorph);
    if (!stage.beetleController) { return; }
    stage.beetleController.beetle.turnAroundWorldAxis(axis,degrees);
});

SnapExtensions.primitives.set('bb_changerot(x, y, z)', function (x, y, z) {
    var stage = this.parentThatIsA(StageMorph);
    if (!stage.beetleController) { return; }
    stage.beetleController.beetle.changeRotations(x, y, z);
});

SnapExtensions.primitives.set('bb_rotateInTurtlePlane(degrees)', function (degrees) {
    var stage = this.parentThatIsA(StageMorph);
    if (!stage.beetleController) { return; }
    stage.beetleController.beetle.rotateInTurtlePlane(degrees);
});
...
SnapExtensions.primitives.set('bb_goWithTurtle(withTurtle)', function (withTurtle) {
    var stage = this.parentThatIsA(StageMorph);
    if (!stage.beetleController) { return; }
        stage.beetleController.beetle.setGoWithTurtle(withTurtle);
});

SnapExtensions.primitives.set('bb_ifGoWithTurtle()', function () {
    var stage = this.parentThatIsA(StageMorph);
    if (!stage.beetleController) { return; }
    return stage.beetleController.beetle.ifGoWithTurtle();
});

SnapExtensions.primitives.set('bb_getTrack()', function () {
    var stage = this.parentThatIsA(StageMorph);
    if (!stage.beetleController) { return; }
    return stage.beetleController.beetle.getTrack();
});

SnapExtensions.primitives.set('bb_2DPositionOf(position)', function (position) {
    var stage = this.parentThatIsA(StageMorph);
    if (!stage.beetleController) { return; }
    return stage.beetleController.beetle.convertWorldToScreen(position.asArray());
});

SnapExtensions.primitives.set('bb_get2DPosition()', function () {
    var stage = this.parentThatIsA(StageMorph);
    if (!stage.beetleController) { return; }
    return stage.beetleController.beetle.convertWorldToScreen(stage.beetleController.beetle.getPosition().asArray());
});

SnapExtensions.primitives.set('bb_get2DTrackInScene()', function () {
    var stage = this.parentThatIsA(StageMorph);
    if (!stage.beetleController) { return; }
    return stage.beetleController.beetle.get2DTrackInScene();
});

SnapExtensions.primitives.set('bb_rotateByXY(deltaX,deltaY)', function (deltaX,deltaY) {
    var stage = this.parentThatIsA(StageMorph);
    if (!stage.beetleController) { return; }
